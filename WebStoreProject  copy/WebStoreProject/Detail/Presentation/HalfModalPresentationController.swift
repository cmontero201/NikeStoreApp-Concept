...
class HalfModalPresentationController : UIPresentationController {
    ...
    func changeScale(to state: ModalScaleState) {
        if let presentedView = presentedView, let containerView = self.containerView {
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseInOut, animations: { () -> Void in
                presentedView.frame = CGRect(x: 0, y: UIScreen.main.bounds.height, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height)
                let containerFrame = CGRect(x: 0, y: UIScreen.main.bounds.height, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height)
                let halfFrame = CGRect(origin: CGPoint(x: 0, y: UIScreen.main.bounds.height),
                                       size: CGSize(width: containerFrame.width, height: UIScreen.main.bounds.height))
                let frame = state == .adjustedOnce ? halfFrame : halfFrame
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        return CGRect(x: 0, y: containerView!.bounds.height / 2.8, width: containerView!.bounds.width, height: containerView!.bounds.height )
    }
